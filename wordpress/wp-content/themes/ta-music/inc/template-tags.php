...
function ta_music_posted_on() {
	...
	$byline = sprintf(
		'<i class="fa fa-user"></i>' . _x( 'Author: %s', 'post author', 'ta_music' ),
	...
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Leave a comment', 'ta_music' ), __( '1 Comment', 'ta_music' ), __( '% Comments', 'ta_music' ) );
		echo '</span>';
	}
	...
	if ( 'post' == get_post_type() ) {
		...
		$categories_list = get_the_category_list( __( ', ', 'ta_music' ) );
...
function ta_music_entry_footer() {
	...
	edit_post_link( __( 'Edit', 'ta_music' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( __( 'Category: %s', 'ta_music' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( __( 'Tag: %s', 'ta_music' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( __( 'Author: %s', 'ta_music' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = sprintf( __( 'Year: %s', 'ta_music' ), get_the_date( _x( 'Y', 'yearly archives date format', 'ta_music' ) ) );
	} elseif ( is_month() ) {
		$title = sprintf( __( 'Month: %s', 'ta_music' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'ta_music' ) ) );
	} elseif ( is_day() ) {
		$title = sprintf( __( 'Day: %s', 'ta_music' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'ta_music' ) ) );
	} elseif ( is_tax( 'post_format' ) ) {
		if ( is_tax( 'post_format', 'post-format-aside' ) ) {
			$title = _x( 'Asides', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
			$title = _x( 'Galleries', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
			$title = _x( 'Images', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
			$title = _x( 'Videos', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
			$title = _x( 'Quotes', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
			$title = _x( 'Links', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
			$title = _x( 'Statuses', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
			$title = _x( 'Audio', 'post format archive title', 'ta_music' );
		} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
			$title = _x( 'Chats', 'post format archive title', 'ta_music' );
		}
	} elseif ( is_post_type_archive() ) {
		$title = sprintf( __( 'Archives: %s', 'ta_music' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
		$title = sprintf( __( '%1$s: %2$s', 'ta_music' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
		$title = __( 'Archives', 'ta_music' );
	}
