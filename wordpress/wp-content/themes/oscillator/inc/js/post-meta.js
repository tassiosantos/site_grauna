jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).on( 'click', '.ci-media-button', function( e ) {
		...
		var ciMediaUpload = wp.media( {
			...
			title: bMulti == true ? oscillator_PostMeta.tSelectFiles : oscillator_PostMeta.tSelectFile,
			button: {
				text: bMulti == true ? oscillator_PostMeta.tUseTheseFiles : oscillator_PostMeta.tUseThisFile
			},
			library: {
				type: bType,
			},
				...
				*/
				...
				if( bMulti == false ) {
					...
					if( target_preview.length > 0 ) {
						...
						var html = '<img src="' + attachment.sizes.thumbnail.url + '" /><a href="#" class="close media-modal-icon" title="' + oscillator_PostMeta.tRemoveImage + '"></a>';
	...
	$( 'body' ).on( 'click', '.ci-upload-to-gallery', function( e ) {
		...
		var ciMediaGallery = wp.media( {
			frame: 'select',
			title: oscillator_PostMeta.tSelectFiles,
			library: {
				type: 'image',
			},
			button: {
				text: oscillator_PostMeta.tUseTheseFiles
			},
			multiple: true
			...
			oscillator_featgal_AJAXUpdate( target_parent );
	...
	function oscillator_featgal_UpdateIDs( preview_element ) {
		var ids = [];
	...
	function oscillator_featgal_AJAXUpdate( gallery_container ) {
		...
		$.ajax( {
			type: 'post',
			url: oscillator_PostMeta.ajaxurl,
			data: {
				action: 'oscillator_featgal_AJAXPreview',
				ids: target_ids.val()
			},
			dataType: 'text',
			beforeSend: function() {
				target_preview.empty().html( '<p>' + oscillator_PostMeta.tLoading + '</p>' );
			},
			success: function( response ) {
				if ( response == 'FAIL' ) {
					target_preview.empty().html( '<p>' + oscillator_PostMeta.tPreviewUnavailable + '</p>' );
				} else {
					...
					$.each( response, function( key, value ) {
						$( '<div class="thumb"><img src="' + value.url + '" data-img-id="' + value.id + '"><a href="#" class="close media-modal-icon" title="' + oscillator_PostMeta.tRemoveFromGallery + '"></a></div>' ).appendTo( target_preview );
					} );
				}
			}//success
		});//ajax
	...
	$( 'body' ).on( 'click', '.ci-media-manager-gallery .thumb a.close', function( event ) {
		...
		oscillator_featgal_UpdateIDs( container );
	} );
	...
	if ( preview_areas.length > 0 ) {
		preview_areas.sortable( {
			update: function( event, ui ) {
				oscillator_featgal_UpdateIDs( $( this ) );
			}
		} );
	}
