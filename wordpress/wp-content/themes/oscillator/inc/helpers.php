<?php
function oscillator_admin_notice_sample_content() {
	...
	?>
	<div class="notice notice-info is-dismissible oscillator-sample-content-notice">
		<p>
			<?php echo sprintf( __( 'It looks like the theme was just installed. <a href="%s" target="_blank">Download the sample content</a> to get things moving.', 'oscillator' ),
				esc_url( 'https://www.cssigniter.com/sample_content/' . CI_THEME_NAME . '.zip' )
			); ?>
		</p>
	</div>
	<script type="text/javascript">
		jQuery( document ).ready( function( $ ) {
			$( '.oscillator-sample-content-notice' ).on( 'click', '.notice-dismiss', function( e ) {
				$.ajax( {
					type: 'post',
					url: ajaxurl,
					data: {
						action: 'oscillator_dismiss_sample_content',
						nonce: '<?php echo wp_create_nonce( 'oscillator-dismiss-sample-content' ); ?>',
						dismissed: true
					},
					dataType: 'text',
...
function oscillator_ajax_dismiss_sample_content() {
	check_ajax_referer( 'oscillator-dismiss-sample-content', 'nonce' );
...
function oscillator_wrap_archive_widget_post_counts_in_span( $output ) {
	$output = preg_replace_callback( '#(<li>.*?<a.*?>.*?</a>.*&nbsp;)(\(.*?\))(.*?</li>)#', 'oscillator_replace_archive_widget_post_counts_in_span', $output );

	return $output;
}

function oscillator_replace_archive_widget_post_counts_in_span( $matches ) {
...
function oscillator_wrap_category_widget_post_counts_in_span( $output, $args ) {
	if ( ! isset( $args['show_count'] ) || $args['show_count'] == 0 ) {
		return $output;
	}
	$output = preg_replace_callback( '#(<a.*?>\s*)(\(.*?\))#', 'oscillator_replace_category_widget_post_counts_in_span', $output );

	return $output;
}

function oscillator_replace_category_widget_post_counts_in_span( $matches ) {
 ...
 */
function oscillator_get_page_var( $default_return = 0 ) {
 ...
 */
function oscillator_get_image_src( $image_id, $size = 'full' ) {
 ...
 * @return string
 */
function oscillator_substr_left( $string, $length ) {
 ...
 * @return string
 */
function oscillator_substr_right( $string, $length ) {
	return substr( $string, - $length, $length );
}

if ( ! function_exists( 'oscillator_get_related_posts' ) ):
/**
 ...
 */
function oscillator_get_related_posts( $post_id, $related_count, $args = array() ) {
...
$oscillator_glob_inline_js = array();
if ( ! function_exists( 'oscillator_add_inline_js' ) ):
/**
 ...
 */
function oscillator_add_inline_js( $script, $handle = false ) {
	global $oscillator_glob_inline_js;

	$handle = sanitize_key( $handle );

	if ( ( $handle !== false ) and ( $handle != '' ) ) {
		$oscillator_glob_inline_js[ $handle ] = "\n" . $script . "\n";
	} else {
		$oscillator_glob_inline_js[] = "\n" . $script . "\n";
	}
}
endif;

if ( ! function_exists( 'oscillator_get_inline_js' ) ):
/**
 * Retrieves the inline JS scripts that are registered for printing.
 *
 * @return array The inline JS scripts queued for printing.
 */
function oscillator_get_inline_js() {
	global $oscillator_glob_inline_js;

	return $oscillator_glob_inline_js;
}
endif;

if ( ! function_exists( 'oscillator_print_inline_js' ) ):
/**
 * Prints the inline JS scripts that are registered for printing, and removes them from the queue.
 */
function oscillator_print_inline_js() {
	global $oscillator_glob_inline_js;

	if ( empty( $oscillator_glob_inline_js ) ) {
		return;
	}

	$sanitized = array();

	foreach ( $oscillator_glob_inline_js as $handle => $script ) {
...
if ( ! function_exists( 'oscillator_inflect' ) ):
	/**
	 ...
	 * @return string
	 */
	function oscillator_inflect( $num, $none, $one, $many ) {
		if ( $num == 0 ) {
			return $none;
		} elseif ( $num == 1 ) {
			return $one;
...
if ( ! function_exists( 'oscillator_e_inflect' ) ):
	/**
	 ...
	 * @return void
	 */
	function oscillator_e_inflect( $num, $none, $one, $many ) {
		echo oscillator_inflect( $num, $none, $one, $many );
	}
endif;

if ( ! function_exists( 'oscillator_merge_wp_queries' ) ):
/**
 ...
 */
function oscillator_merge_wp_queries() {
	$args = func_get_args();
	...
	$params = apply_filters( 'oscillator_merge_wp_queries', $params, $args );
...
if ( ! function_exists( 'oscillator_dropdown_posts' ) ):
/**
 ...
 */
function oscillator_dropdown_posts( $args = '', $name = 'post_id' ) {
	$defaults = array(
		'depth'                 => 0,
		'post_parent'           => 0,
		'selected'              => 0,
	...
	$output = apply_filters( 'oscillator_dropdown_posts', $output, $name, $r );
