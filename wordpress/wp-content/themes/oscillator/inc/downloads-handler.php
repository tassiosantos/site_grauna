...
add_action( 'init', 'oscillator_check_get_for_downloads', 1 );
if ( ! function_exists( 'oscillator_check_get_for_downloads' ) ):
	function oscillator_check_get_for_downloads() {
	...
	if ( empty( $file_url ) ) {
		oscillator_force_download_throw_404();
	...
	// Add URLs like this:
	//	add_filter( 'oscillator_downloads_handler_safe_path', 'oscillator_mod_add_downloads_handler_safe_path' );
	//	function oscillator_mod_add_downloads_handler_safe_path( $paths ) {
	//		$paths[] = array(
	...
	$safe_paths = apply_filters( 'oscillator_downloads_handler_safe_path', $safe_paths );
	...
	if ( $found_in_safe_path !== false ) {
		...
		if ( empty( $abs_file_path ) or strpos( $abs_file_path, $safe_path['dir'] ) === false ) {
			...
			if ( $file_info['extension'] == '.php' || ! is_readable( $abs_file_path ) ) {
				die( __( 'Download Handler: Access to this file is not permitted.', 'oscillator' ) );
			}
	...
	if ( $redirect ) {
		...
		echo esc_html__( 'Download Handler: Unknown error.', 'oscillator' );
		exit;
	}
...
if ( ! function_exists( 'oscillator_force_download_throw_404' ) ):
function oscillator_force_download_throw_404() {
	add_action( 'parse_query', 'oscillator_force_download_throw_404_handler', 1 );
}
endif;

if ( ! function_exists( 'oscillator_force_download_throw_404_handler' ) ):
function oscillator_force_download_throw_404_handler() {
	global $wp_query;
	$wp_query->set_404();
	status_header( 404 );
}
